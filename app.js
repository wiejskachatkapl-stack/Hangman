...
function finish(win){
  ...
  if(win){
    ...
    renderGame('');
    save();
    // v1087: pokazujemy okno po odgadnięciu natychmiast i z zapasem,
    // żeby nie zgubiło się po odświeżeniu renderu albo na telefonie.
    showWinPrompt();
...
applyScale();
if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=1087').catch(()=>{}));}
