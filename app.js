...
function updatePlayHotspots(){
  ...
  let scale = Math.min(rect.width / PLAY_BG_SIZE.w, rect.height / PLAY_BG_SIZE.h);
  const desktopWide = window.matchMedia && window.matchMedia('(orientation: landscape) and (min-width: 901px) and (min-height: 521px)').matches;
  if(desktopWide) scale *= 1.02;
...
applyScale();
if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=1073').catch(()=>{}));}
