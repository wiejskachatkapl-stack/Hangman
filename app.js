...
function updatePlayHotspots(){
  ...
  const baseScale = Math.min(rect.width / PLAY_BG_SIZE.w, rect.height / PLAY_BG_SIZE.h);
  const scale = baseScale * 1.04; // v1071: delikatne rozszerzenie tła GRAJ w lewo i prawo
...
applyScale();
if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('sw.js?v=1071').catch(()=>{}));}
